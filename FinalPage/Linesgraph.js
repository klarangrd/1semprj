...
d3.json("http://localhost:3000/population") 
    .then(function (data) {
        ...
        const yScale = d3.scaleLinear()
            .domain([0, d3.max(lineData, d => Math.max(d.lionfish_pop, d.shark_pop))]) //setting the y scale to be the max of the data
            .range([height, 0]);
        ...
        function updateTooltip() {
            ...
            let value = id == "shark" ? dataPoint.shark_pop : dataPoint.lionfish_pop; //value should be shark if sharkpop otherwise lionfish

            tooltip.html(`<strong>Year:</strong> ${dataPoint.year}<br><strong>${id == "shark" ? "Shark" : "Lionfish"} Population:</strong>${value}`) //value 
